...
def detect_dimensions(df: pd.DataFrame) -> list[str]:
    ...
    # Las dimensiones en config.DIM_PRIORITY van primero, en ese orden; el
    # resto mantiene el orden en que aparecen en el Sheet (sort estable).
    prioridad = config.DIM_PRIORITY
    dims.sort(key=lambda d: prioridad.index(d) if d in prioridad else len(prioridad))
    return dims
